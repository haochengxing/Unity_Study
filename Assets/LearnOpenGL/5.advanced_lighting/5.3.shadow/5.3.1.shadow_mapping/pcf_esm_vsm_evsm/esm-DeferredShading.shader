Shader "Unlit/esm-DeferredShading"
{
    Properties
    {
        _LightTexture0("",any) = "" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float myEsm(float4 lightScreenPos,float d){
                ...
                for(int i=-2;i<=2;i++){
                    for(int j=-2;j<=2;j++){
                        float z = tex2Dlod(esmTex,float4(lightScreenPos.xy+float2(j,i)/shadowmapSize,0,0)).r;
            ...
            half4 CalculateLight(unity_v2f_deferred i){
                ...
                half onMinusReflectivity = 1-SpecularStrength(data.specularColor.rgb);
                ...
                half4 res = UNITY_BRDF_PBS(data.diffuseColor,data.specularColor,onMinusReflectivity,data.smoothness,data.normalWorld,-eyeVec,light,ind);
